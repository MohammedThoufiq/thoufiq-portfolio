...
export default function Navbar() {
  ...
  return (
    <motion.nav
      initial={false}
      animate={{
        backgroundColor: solid ? activeBg : transparentBg,
        backdropFilter: solid ? "blur(20px)" : "blur(0px)",
      }}
      transition={{ duration: 0.25 }}
      className={`
        fixed top-0 left-0 right-0 z-[9999] w-full border-b transition-colors
        ${solid ? "border-hairline" : "border-transparent"}
      `}
    >
        ...
        <div className="flex items-center justify-between py-4">
          ...
          <Link
            ...
            className="
              ...
              text-base md:text-lg font-semibold text-fg
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden flex flex-col gap-3 pb-6"
            >
              {navLinks.map((link) => (
                <Link
                  key={link.href}
